...
def get_middle_frame(scene_id, gt_class, video_path=None):
    """Find and load the middle frame from exported frames or video file."""
    # Try exported frames directory
    if FIRE_FRAMES_DIR.exists():
        frames = sorted(FIRE_FRAMES_DIR.glob(f"{scene_id}_*.jpg"))
        if frames:
    ...
    if video_path and Path(video_path).exists():
        ...
        for sub in ["preprocessed", "samples/part1", "samples/part2", "samples/part3", ""]:
            for ext in [".mp4", "_prep.mp4"]:
                candidate = FIRE_DATASET_DIR / sub / f"{scene_id}{ext}"
                if candidate.exists():
                    vp = candidate
                    break
            if vp:
...
def main():
    ...
    if not RESULTS_COMBINED.exists():
        print(f"  ERROR: Results not found: {RESULTS_COMBINED}")
        return
    ...
    for r in data.get("results", []):
        ...
        # Save origin image
        output_path = REPORTS_DIR / f"origin_{scene_id}.jpg"
        imwrite_unicode(output_path, img)
        print(f"    Saved: {output_path}")
        generated.append(str(output_path))

        # Save temporal strip
        strip_path = REPORTS_DIR / f"temporal_{scene_id}.jpg"
