...
from src.config import FIRE_DATASET_DIR, REPORTS_DIR, MARKUP_PATH, RESULTS_2B, RESULTS_NEW, RESULTS_COMBINED
...
def collect_videos():
    ...
    videos = []
    prep_dir = FIRE_DATASET_DIR / "preprocessed"

    # From archive.zip samples
    for part_dir in sorted(FIRE_DATASET_DIR.glob("samples/part*")):
    ...
    for mp4 in sorted(FIRE_DATASET_DIR.glob("*.mp4")):
        video_name = mp4.stem
...
def merge_all_results():
    ...
    combined_path = RESULTS_COMBINED
