...
def export_representative_frames(results_data=None):
    ...
    FIRE_FRAMES_DIR.mkdir(parents=True, exist_ok=True)

    exported = {}

    # Use existing exported frames
    if FIRE_FRAMES_DIR.exists():
        all_frames = sorted(FIRE_FRAMES_DIR.glob("*.jpg"))
        scene_map = {}
        for fp in all_frames:
            parts = fp.stem.rsplit("_f", 1)
            if len(parts) == 2:
                scene_key = parts[0].rsplit("_", 1)
                if len(scene_key) == 2:
                    sid, gt = scene_key
                    scene_map.setdefault(sid, {"gt_class": gt, "frames": []})
                    idx = int(parts[1])
                    scene_map[sid]["frames"].append((fp, idx))

        for scene_id, info in scene_map.items():
            info["frames"].sort(key=lambda x: x[1])
            exported[scene_id] = {
    ...
    if results_data:
        for r in results_data.get("results", []):
            ...
            scene_frames = []
            for idx in indices:
                dst = FIRE_FRAMES_DIR / f"{scene_id}_{gt_class}_f{idx:04d}.jpg"
                if not dst.exists():
...
def build_firetrace_dataset():
    ...
    if not RESULTS_COMBINED.exists():
        print(f"    ERROR: {RESULTS_COMBINED} not found")
        return None

    with open(RESULTS_COMBINED, "r", encoding="utf-8") as f:
        data = json.load(f)
...
def build_origin_overlay_dataset():
    ...
    # Add temporal strips
    temporal_images = sorted(REPORTS_DIR.glob("temporal_*.jpg"))
